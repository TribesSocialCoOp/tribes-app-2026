...
test.describe('FOSS Password Authentication Flows', () => {
  test('Signup page - should support password selector, live validation, and security callout', async ({ page }) => {
    ...
    // Wait until either the password toggle button or the password input is visible
    await Promise.race([
      page.waitForSelector('button:has-text("I prefer to use a password")', { timeout: 10000 }).catch(() => {}),
      page.waitForSelector('input[type="password"]', { timeout: 10000 }).catch(() => {})
    ]);

    // Toggle to password mode if available
    const passwordTab = page.locator('button:has-text("I prefer to use a password")');
    if (await passwordTab.isVisible()) {
      await passwordTab.click();
    }

    // Verify fields are visible in password mode
    ...
    await expect(page.locator('text=Vault Backup Warning')).toBeVisible();
  ...
  test('Login page - should support switching to password fallback and back', async ({ page }) => {
    ...
    // Wait until either the password toggle button or the password input is visible
    await Promise.race([
      page.waitForSelector('button:has-text("I use a password")', { timeout: 5000 }).catch(() => {}),
    ...
    if (await passwordTab.isVisible()) {
      ...
      const passkeyTab = page.locator('button:has-text("I use a passkey")');
