...
**If you use a passkey with biometric unlock (FaceID, TouchID, Windows Hello),** you probably won't see this banner at all. Your keys are automatically backed up every time new keys are generated. The banner is suppressed when automatic PRF vault sync is active.
...
If you do see this banner (e.g., you're on a browser without passkey PRF support):
...
### Automatic: Biometric Sync (Recommended)

If you log in with a passkey (FaceID, TouchID, Windows Hello), **your keys are backed up automatically.** Every time a new encryption key is generated, it's silently encrypted and saved to the server using a hardware-derived secret from your passkey. You'll see a "Key Sync Enabled" confirmation the first time this happens.

When you log in on a new device with the same passkey, your keys are automatically restored. You'll see a "Security Synced" notification and everything just works. No passphrase needed. No manual steps.

This works because your passkey's secure enclave produces a deterministic secret (via the WebAuthn PRF extension) that we use to encrypt your vault. The server never sees the secret. Only your biometric unlock can produce it.

### Manual: Passphrase Backup (Fallback)

If your browser or authenticator doesn't support passkey PRF (older security keys, some browsers), you can manually back up your vault with a passphrase:
...
**When to manually back up:**
- After you create a new bond (if not using biometric sync)
- After you switch devices
- Before you clear browser data
- Whenever the app shows the blue banner

If you're on multiple devices, the vault merges keys from all of them. Back up from Device A, restore on Device B, and both devices have all your keys.
...
|---|---|---|
| "Key Sync Enabled" toast | PRF auto-backup just saved your keys | Nothing — it's automatic from now on |
| "Security Synced" toast | Keys restored from your passkey vault | Nothing — you're good to go |
| "Encryption Keys Missing" banner (amber) | A bond key was generated on another device | Restore from vault backup, or reset keys |
| "Vault backup recommended" banner (blue) | Keys are healthy but not backed up (no PRF) | Go to Settings > Key Vault and back up |
...
| Everything green, no banners | All keys synced, encryption active | You're good. PRF users: fully automatic. |
