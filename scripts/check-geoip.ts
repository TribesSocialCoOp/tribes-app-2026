...
import { regionTier } from '../src/lib/geo/age-policy';
...
const SAMPLE: Array<[string, string]> = [
  ...
  ['129.7.0.1', 'US-TX (verify — law state)'],
  ['164.119.0.1', 'US Midwest (verify — law state)'],
...
async function main() {
  ...
  const resolve = (ip: string): { code: string; tier: string } => {
    try {
      ...
      return { code: code || '(unknown)', tier: regionTier(code) };
    } catch {
      return { code: '(not found)', tier: regionTier('') };
    }
  };
  ...
  for (const [ip, note] of rows) {
    const { code, tier } = resolve(ip);
    const mark = tier === 'blocked' ? '⛔' : tier === 'verify' ? '🪪' : '✅';
    console.log(`${mark} ${ip.padEnd(16)} → ${code.padEnd(8)} [${tier}]${note ? `   ${note}` : ''}`);
  }
}
